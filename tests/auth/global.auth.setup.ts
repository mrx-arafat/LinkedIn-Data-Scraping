...
function isValidStorageFile(p: string): boolean {
  try {
    ...
    const obj = JSON.parse(fs.readFileSync(p, 'utf-8')) as { cookies?: any[]; origins?: any[] };
...
setup('authenticate to LinkedIn and save storage', async ({ page }, testInfo) => {
  ...
  while (Date.now() - started < 90_000) {
    const cookies = await page.context().cookies({ urls: ['https://www.linkedin.com'] });
    if (cookies.some((c) => c.name === 'li_at')) { hasLiAt = true; break; }
    await page.waitForTimeout(2000);
  }

  if (!hasLiAt) {
    testInfo.annotations.push({ type: 'info', description: 'Manual login likely required. Complete any checkpoints, then resume.' });
  ...
  const finalCookies = await page.context().cookies({ urls: ['https://www.linkedin.com'] });
