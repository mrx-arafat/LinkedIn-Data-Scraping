...
// Use the shared Playwright auth state generated by the login setup
...
function toAbs(url: string | null): string | null {
  ...
  return url.startsWith("http")
    ? url
    : new URL(url, "https://www.linkedin.com").toString();
}
...
async function navigateToRecentActivityAll(page: any, username: string) {
  ...
  const isAuthwall =
    page.url().includes("/authwall") || page.url().includes("/login");
  if (isAuthwall) {
    console.log(
      "⚠️  Auth wall detected. Please run the login script first: npx tsx tests/auth/manual-login.ts"
    );
...
async function collectPostFromItem(
  page: any,
  node: any,
  ...
  try {
    ...
    const postText =
      cleanText(
        await node
          .locator('.feed-shared-text, .update-components-text, [dir="ltr"]')
          .first()
          .innerText()
          .catch(() => "")
      ) || null;
    ...
    const authorName = cleanText(
      await node
        .locator(
          '.update-components-actor__name, .feed-shared-actor__name, [data-control-name="actor"] span'
        )
        .first()
        .innerText()
        .catch(() => sourceUsername)
    );
    ...
    // Media - simplified
    const images: string[] = [];
    const imgElements = await node
      .locator(".update-components-image img, .feed-shared-image img")
      .all()
      .catch(() => []);
      ...
      for (let i = 0; i < posts.length && items.length < targetCount; i++) {
        try {
          ...
          const postData = await page.evaluate((el: any) => {
            ...
            for (const sel of authorSelectors) {
              ...
              if (authorEl && authorEl.innerText) {
                ...
                for (const line of lines) {
                  ...
                  if (line.match(/^(Premium|Verified|•|1st|2nd|3rd\+?)$/i))
                    continue;
                  if (line.includes("•")) {
                    ...
                    if (
                      namePart &&
                      !namePart.match(/^(Premium|Verified|1st|2nd|3rd\+?)$/i)
                    ) {
                      author = namePart;
                      break;
                    }
                  } else if (
                    !line.match(/^(You|Premium|Verified|1st|2nd|3rd\+?)$/i)
                  ) {
                    author = line;
                    break;
                  }
            ...
            const subDescEl = el.querySelector(
              ".update-components-actor__sub-description"
            );
            if (subDescEl) {
              const timeText =
                subDescEl.innerText || subDescEl.textContent || "";
              // Extract time pattern like "20m", "4hr", "2d", "1w", "9mo", "1yr"
              const timeMatch = timeText.match(
                /(\d+\s*(?:m(?:in)?|h(?:r)?|d(?:ay)?s?|w(?:eek)?s?|mo(?:nth)?s?|y(?:r|ear)?s?))\s*(?:ago|•)/i
              );
              if (timeMatch) {
            ...
            if (!postedTime) {
              ...
              if (timeEl) {
                ...
                const timeMatch = timeText.match(
                  /(\d+\s*(?:m(?:in)?|h(?:r)?|d(?:ay)?s?|w(?:eek)?s?|mo(?:nth)?s?|y(?:r|ear)?s?))/i
                );
                if (timeMatch) {
                  postedTime = timeMatch[1].replace(/\s+/g, "");
                } else {
          ...
          const post = {
            ...
            postUrl: postData.postId
              ? `https://www.linkedin.com/feed/update/urn:li:activity:${postData.postId}/`
              : null,
          ...
          items.push(post);
          console.log(
            `  ✅ Collected post ${items.length}: "${post.postText.substring(
              0,
              50
            )}..."`
          );
          ...
          if (items.length >= targetCount) {
            console.log(
              `  🎯 Reached target count of ${targetCount} posts for ${username}`
            );
            break;
          }
        } catch (err) {
          console.log(
            `  ⚠️ Error processing post ${i}:`,
...
function reorderForCSV(rows: any[]) {
  ...
  return rows.map((r) => {
    ...
    const attachmentsTitles = (r.attachments || [])
      .map((a: any) => a.title ?? "")
      .join(";");
    const attachmentsUrls = (r.attachments || [])
      .map((a: any) => a.url ?? "")
      .join(";");
    return {
...
test.describe("LinkedIn Recent Activity - Posts scraper", () => {
  ...
  test("scrape recent-activity posts for usernames", async ({ page }) => {
    ...
    for (const username of USERNAMES) {
      ...
      try {
        ...
        userStats[username] = {
          ...
          postsWithMedia: items.filter(
            (p: any) => p.media.images.length > 0 || p.media.videos.length > 0
          ).length,
          avgReactions:
            items.reduce(
              (sum: number, p: any) => sum + (p.reactionsCount || 0),
              0
            ) / items.length || 0,
          avgComments:
            items.reduce(
              (sum: number, p: any) => sum + (p.commentsCount || 0),
              0
            ) / items.length || 0,
        };
        ...
        console.log(
          `   Total posts collected: ${items.length}/${SCRAPING_CONFIG.maxPosts}`
        );
        console.log(
          `   Posts with media: ${userStats[username].postsWithMedia}`
        );
        console.log(
          `   Avg reactions: ${Math.round(userStats[username].avgReactions)}`
        );
        console.log(
          `   Avg comments: ${Math.round(userStats[username].avgComments)}`
        );
      } catch (error) {
    ...
    const jsonPath = path.join(
      OUT_DIR,
      `posts-${usernamesStr}-${timestamp}.json`
    );
    const csvPath = path.join(
      OUT_DIR,
      `posts-${usernamesStr}-${timestamp}.csv`
    );
